...
import { createPasantiaDTO, sanitizarPasantiaDTO, updatePasantiaDTO } from './pasantias.dto';
...
export class PasantiasSService {
    ...
    //GETS
    async total_rows(pais:string, search:string){
        const count = await this.pasantiaRepository.count({where:{
            titulo:Like(`%${search}%`),
            isActive:true,
            pais:pais
        }})
    ...
    //pasantias busqueda precisa
    async search_pasantias(content: string, pais: string, page: number, limit: number) {
        ...
        if (pasantias.length <= 0) throw new NotFoundException('Not found pasantias');
        const sanitized_pasantias = await Promise.all(pasantias.map(async pasantia => {
            const sanitized = await new sanitizarPasantiaDTO().sanitizar(pasantia)
            return{...sanitized,user_name:pasantia.user?.nom_usuario};
        }
        ));
        return sanitized_pasantias;
    }
