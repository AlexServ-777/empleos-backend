...
import { createEmpleoDTO, sanitizarEmpleoDTO, updateEmpleoDTO } from './empleos.dto';
...
export class EmpleosSService {
    ...
    //gets
    async get_total_empleo(pais: string, search: string) {
        const total = await this.empleosRepository.count({ where: { isActive: true, pais: pais, titulo: Like(`%${search}%`) } });
        return total;
    }

    //empleos con busqueda exacta y paginacion
    async search_empleos(pais: string, content: string, page: number, limit: number) {
        if (!pais || !page || page === 0) throw new ConflictException('page number is empty or pais is undefined');
        ...
        if (empleos.length <= 0) throw new NotFoundException('not found empleos');
        const sanitizar = await Promise.all(empleos.map(async(empleo) => {
            const sanitized = await new sanitizarEmpleoDTO().sanitizar(empleo)
            return {...sanitized,user_name:empleo.user.nom_usuario}
        }));
        return sanitizar;
    }
