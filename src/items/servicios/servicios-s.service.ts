...
import { createServicioDTO, sanitizarServicioDTO, updateServicioDTO } from 'src/items/servicios/servicios.dto';
...
export class ServiciosSService {
    ...
    //GETS
    async count_total_rows(pais:string,search:string){
        const count = await this.serviciosRepository.count({where:{
            titulo:Like(`%${search}%`),
            isActive:true,
            pais:pais
        }});
    ...
    async search_servicios(pais:string, content:string, page:number, limit:number){
        if(!pais||!page||page===0) throw new ConflictException('page is empty or pais is undefined');
        ...
        if(servicios.length<=0) throw new NotFoundException('not found more servicios');
        const servicios_sanitized = await Promise.all(servicios.map(async (servicio)=> {
            const sanitized = await new sanitizarServicioDTO().sanitizar(servicio);
            return {...sanitized, user_name:servicio.user.nom_usuario}
        }));
        return servicios_sanitized;
    }
